...
class FullscreenSwipeTest {
    ...
    fun `upward swipe beyond threshold opens the next video`() {
        assertEquals(2, fullscreenSwipeTargetIndex(activeIndex = 1, itemCount = 4, offsetPx = -240f, viewportHeightPx = 1_000))
    }

    @Test
    fun `downward swipe beyond threshold opens the previous video`() {
        assertEquals(0, fullscreenSwipeTargetIndex(activeIndex = 1, itemCount = 4, offsetPx = 240f, viewportHeightPx = 1_000))
    }

    @Test
    fun `short drag springs back without switching`() {
        assertNull(fullscreenSwipeTargetIndex(activeIndex = 1, itemCount = 4, offsetPx = -180f, viewportHeightPx = 1_000))
    }
